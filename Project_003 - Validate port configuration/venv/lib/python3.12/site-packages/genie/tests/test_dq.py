...
class test_dq(unittest.TestCase):
    ...
    def test_get_value_conditions_key_value(self):
        mod = Dq(module)
        # one condition -> one hit
        self.assertEqual(mod.get_values('model', conditions_key_value={'serial_number': 'TM40010000B'}), ['N7K-SUP1'])

        # two conditions -> one hit
        self.assertEqual(mod.get_values('model', conditions_key_value={'serial_number': 'TM40010000B', 'status': 'active'}), ['N7K-SUP1'])

        # three conditions -> one hit
        self.assertEqual(mod.get_values('model', conditions_key_value={'serial_number': 'TM40010000B', 'status': 'active', 'software': '7.3(0)D1(1)'}), ['N7K-SUP1'])

        # one condition -> multiple hits
        self.assertEqual(mod.get_values('model', conditions_key_value={'status': 'ok'}), ['N7K-F248XP-25', 'N7K-F248XP-25', 'N7K-F248XP-25'])

        # two conditions -> multiple hits
        self.assertEqual(mod.get_values('model', conditions_key_value={'status': 'ok', 'slot/world_wide_name': '--'}), ['N7K-F248XP-25', 'N7K-F248XP-25', 'N7K-F248XP-25'])

        # one condition -> one hit with index
        self.assertEqual(mod.get_values('model', conditions_key_value={'serial_number': 'TM40010000B'}, index=0), 'N7K-SUP1')

        # two condition -> no hit
        self.assertEqual(mod.get_values('model', conditions_key_value={'serial_number': 'TM40010000B', 'status': 'ok'}), [])
