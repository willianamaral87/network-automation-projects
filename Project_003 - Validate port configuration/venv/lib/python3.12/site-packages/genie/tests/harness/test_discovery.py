...
class TestTestScriptDiscovery(unittest.TestCase):
    ...
    def test_triggers_exclude_uids(self):
        # Test excluding triggers using exclude_trigger_uids
        self.script.trigger_uids = Or('TriggerShutNoShutBgp', 'TriggerShutNoShutOspf')
        self.script.exclude_trigger_uids = ['TriggerShutNoShutOspf']
        self.script.trigger_groups = None
        # Add triggers information to the script
        self.script.triggers = self.triggers8
        self.script.subsection_data = self.subsections1
        items = list(self.discoverer)
        # Should only have TriggerShutNoShutBgp, not TriggerShutNoShutOspf
        for expected, item in zip(
            [CommonSetup, TriggerShutNoShutBgp, CommonCleanup], items):
            self.assertTrue(isinstance(item, expected))

    def test_triggers_exclude_uids_with_logic(self):
        # Test excluding triggers using And/Or logic
        self.script.trigger_uids = Or('TriggerShutNoShutBgp', 'TriggerShutNoShutOspf')
        self.script.exclude_trigger_uids = Or('TriggerShutNoShutOspf')
        self.script.trigger_groups = None
        # Add triggers information to the script
        self.script.triggers = self.triggers8
        self.script.subsection_data = self.subsections1
    ...
    def test_verifications_exclude_uids(self):
        # Test excluding verifications using exclude_verification_uids
        self.script.verification_uids = Or('a_callable', 'a_callable_always_same')
        self.script.exclude_verification_uids = ['a_callable_always_same']
        self.script.trigger_uids = Or('')
        # Add triggers information to the script
        self.script.verifications = self.veri1
        self.script.subsection_data = self.subsections1
        ...
        # Should only have a_callable, not a_callable_always_same
        self.assertEqual(items[1].uid, 'Verifications.post')
        self.assertEqual(len(items[1].verifiers), 1)
        self.assertEqual(items[1].verifiers[0].uid, 'a_callable.PE1.1')

    def test_verifications_exclude_uids_with_logic(self):
        # Test excluding verifications using And/Or logic
        self.script.verification_uids = Or('a_callable', 'a_callable_always_same')
        self.script.exclude_verification_uids = Or('a_callable_always_same')
        self.script.trigger_uids = Or('')
        # Add verifications information to the script
        self.script.verifications = self.veri1
        self.script.subsection_data = self.subsections1
        items = list(self.discoverer)
    ...
    def test_extended_trigger_entrypoint(self, mock_entry_points):
        ...
        try:
            ...
            _dist = distribution("genie.libs.cisco")
            for entry_point in get_entry_points(TRIGGER_ENTRY_POINT):
                if "cisco" in entry_point.name:
                    break
            else:
                raise ImportError()
        except ImportError:
        ...
        self.assertEqual(entry_point.module, "genie.libs.cisco.sdk.triggers")
