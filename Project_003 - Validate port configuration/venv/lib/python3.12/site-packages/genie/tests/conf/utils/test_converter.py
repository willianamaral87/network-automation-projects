...
class test_converter(unittest.TestCase):
    ...
    def test_interface(self):
        ...
        # iosxe has a default tunnel_mode of 'gre' for tunnel type interfaces
        device.os = 'iosxe'
        intf = Converter.convert_interface(dev1.interfaces['Tunnel10'], device)
        self.assertIsInstance(intf, BaseInterface)
        self.assertEqual(intf.tunnel_mode, 'gre')

    def test_interface_tunnel_mode_defined(self):
        '''Can it figure out the right interface?
        ...
        intf = Converter.convert_interface(dev1.interfaces['Tunnel10'], device)
        self.assertEqual(intf.tunnel_mode, 'mpls traffic-eng')
