...
class TestParserWithConnectionPool(unittest.TestCase):
    ...
    def test_parser_with_connection_pool(self):
        ...
        mock_device.type = 'router'
        mock_device.name = 'Router'
        
        # Create mock connections for the pool
        mock_cli = MagicMock()
        mock_cli.connected = True
        mock_cli_1 = MagicMock()
        mock_cli_1.connected = True
        
        # Track which connections were created/used
        connections_created = []
        
        def mock_instantiate(connection_alias, *args, **kwargs):
            conn = mock_cli if connection_alias == 'cli' else mock_cli_1
            connections_created.append(connection_alias)
            return conn
        
        mock_device.instantiate = mock_instantiate
        mock_device.connections = {
            'cli': mock_cli,
            'cli_1': mock_cli_1
        }
        
        # Mock the connect method to simulate pool creation
        def mock_connect(pool_size=1, **kwargs):
            # Simulate creating pool_size connections
            for i in range(pool_size):
                alias = 'cli' if i == 0 else f'cli_{i}'
                if alias not in connections_created:
                    mock_instantiate(alias)
        
        mock_device.connect = mock_connect
        
        # Connect with pool_size=2
        r = mock_device
        ...
        *    1 65    C9300-48U          17.18.01          CAT9K_IOSXE           INSTALL
        ...
        # Mock parse to return the expected parsed output
        r.parse = MagicMock(return_value=parsed_output)
        
        # Test the parser functionality
        out = r.parse('show version', output=output, timeout=10)
        
        # Verify pool was created with 2 connections
        self.assertEqual(len(connections_created), 2)
        self.assertIn('cli', connections_created)
        self.assertIn('cli_1', connections_created)
        
        # Verify parse was called correctly
        r.parse.assert_called_once_with('show version', output=output, timeout=10)
