...
class Base(_genie_Base):
    ...
    @classmethod
    def _get_abstract_subclass(cls, device):
        '''_get_abstract_subclass

        Find and return the abstracted subclass of this class with the best
        match for the given device. Also searches external conf packages.
        '''
        ...
        PYATS_EXT_CONF_ENV_VAR = PYATS_EXT_CONF.upper().replace('.', '_')
        ext_conf_package = os.environ.get(PYATS_EXT_CONF_ENV_VAR)
        if ext_conf_package:
            pkgs.extend(ext_conf_package.split(','))

        ext_conf_package = cfg.get(PYATS_EXT_CONF, None)
        if ext_conf_package:
            pkgs.extend(ext_conf_package.split(','))

        # Check Conf entrypoints
        eps = get_entry_points(CONF_ENTRYPOINT)
        for entry in eps:
            pkgs.append(entry.module)

        for pkg in pkgs:
            try:
                new_class = load_attribute(pkg,
                                           relative_class_name,
                                           device,
                                           suppress_warnings=True)
            except (LookupError, AttributeError):
                # No external implementation for this class in this external
                # package
                continue

            if not issubclass(new_class, cls):
                # Abstract lookup found something other than a subclass - likely
                # ABC. Attempt direct lookup of this class equivalent in the
                # external package
                try:
                    new_class = load_attribute(None,
                                               f'{pkg}.{relative_class_name}',
                                               suppress_warnings=True)
        ...
        try:
            # Final lookup in genie.libs.conf
            new_class = load_attribute('genie.libs.conf',
                                       relative_class_name,
                                       device,
                                       suppress_warnings=True)
            return new_class
    ...
    def __new__(cls, *args, **kwargs):
        # When creating a new conf object, immediately attempt to get the
        # matching abstracted class for this device
        device = kwargs.get('device', None)
        if device:
            new_class = cls._get_abstract_subclass(device)
            if new_class is not cls and issubclass(new_class, cls):
                return new_class.__new__(new_class, *args, **kwargs)
